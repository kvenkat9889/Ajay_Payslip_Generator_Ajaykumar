...
// Payslip data validator middleware
...
app.post('/api/payslips', validatePayslipData, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Payslip generated successfully', data: insertResult.rows[0] });
    } catch (err) {
        logError('Error creating payslip', err);
